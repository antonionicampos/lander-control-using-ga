...
class EvolutionStrategy:
    ...
    def __init__(self, pop_size=50, hidden_units=[64, 64]):
        ...
        self.policy = NN(input_dim=self.input_dim, 
        ...
        all_units = [self.input_dim, *self.hidden_units, self.output_dim]
        for index in range(1, len(all_units)):
            weights_dim = all_units[index] * all_units[index - 1]
            biases_dim = all_units[index]
            weights = np.random.normal(0., 1., size=(weights_dim, self.pop_size))
            biases = np.zeros((biases_dim, self.pop_size))
            genes = np.r_[weights, biases] if index == 1 else np.r_[genes, weights, biases]
    ...
    def crossover(self, parents):
        children = np.zeros((2*self.dim, self.lambda_))
    ...
    def fitness(self, population):
        episodes = 5
        fitnesses = np.zeros((population.shape[1]))
    ...
    def run(self, n_gens=100, sigma_min=0.00001):
        ...
        best_fitness_per_gen = np.zeros(n_gens)
        avg_fitness_per_gen = np.zeros(n_gens)
        ...
        for gen in range(n_gens):
            ...
            best_individual = self.pop[:, best_indices[-1]]
            ...
            sys.stdout.write(
                ...
                "Memory Usage {} MB\t".format(
                    gen+1, 
                    elapsed_time,
                    np.round(best_fitness_per_gen[gen], 2),
                    np.round(avg_fitness_per_gen[gen], 2), 
